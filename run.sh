...
# load non system databases
if [ "${MYSQL_DB}" == "--user-databases" ]; then
	MYSQL_DB=`mysql -h${MYSQL_HOST} -P${MYSQL_PORT} -u${MYSQL_USER} -p${MYSQL_PASS} ${EXTRA_OPTS} -ANe"SELECT schema_name FROM information_schema.schemata WHERE schema_name NOT IN ('mysql','information_schema','performance_schema', 'test')"`
fi
